...
function calculateEMI() {
    ...
    const rate = Number(document.getElementById("loanRate")?.value) / 1200;
...
function restoreSimulationData() {
    ...
    const saved = JSON.parse(
        localStorage.getItem("finverseSimulations")
    );
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("compoundBtn")
        ?.addEventListener("click", calculateCompound);

    document.getElementById("emiBtn")
        ?.addEventListener("click", calculateEMI);

    document.getElementById("goalBtn")
        ?.addEventListener("click", calculateGoal);
